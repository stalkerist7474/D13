...
def post_author_directory_path(instance, filename):
    ...
    return 'author_post_{0}/{1}'.format(instance.author.id, filename) 
...
class FilePost(models.Model):
    title = models.CharField(max_length = 255)
    file = models.FileField(upload_to=post_author_directory_path, null=True)
    author = models.OneToOneField(User, on_delete = models.CASCADE, null=True)
...
class Post(models.Model):
    ...
    post_images = models.ForeignKey(ImagePost,on_delete = models.CASCADE, null=True, blank=True)
    post_files = models.ForeignKey(FilePost,on_delete = models.CASCADE, null=True, blank=True)
